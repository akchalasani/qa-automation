...
And(/^I click on grocery menu drop down and verify the links present$/) do
  on(VisitorsGoodShopHome) do |page|
    page.click_grocery_menu
    expect(page.grocery_dropdown_links_element.exists?).to be_truthy
  end
end

And(/^I click on goodshop give tab and verify the page displayed$/) do
  on(VisitorsGoodShopHome) do |page|
    page.click_goodshop_give_menu
    expect(@browser.url).eql? "https://uat.goodsearch.com/give"
  end
end